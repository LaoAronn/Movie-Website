    ...
    <body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="myNavbar">
                ...
                <?php if (isset($_COOKIE["uid"])){ ?>
                <li class="active"><a href="user_ratings.php"> <span class="glyphicon glyphicon-star-empty"></span> Your Rated Movies </a></li>
            ...
            <div class="container">
                <div class="row">
                    <?php
                        ...
                        // DISPLAY MOVIE INFO
                        $user = $_COOKIE["uid"];
                        $display_user_rating = "SELECT * FROM movies AS m JOIN ratings AS r ON r.movie_id = m.movie_id JOIN users AS u ON r.user_id = u.user_id WHERE r.user_id = (SELECT user_id FROM users WHERE username = '$user')";
                        $stmt1 = $pdo->prepare($display_user_rating);
                        $stmt1->execute();
                            ...
                            <div class="row_movie">
                                ...
                                <div class="col_movieinfo">
                                    ...
                                    <?php
                                        // QUERY CONSTRUCTOR
                                        $id = $movie['movie_id'];
                                        $ratings = "SELECT rating FROM ratings as r JOIN users AS u ON u.user_id = r.user_id JOIN movies AS m ON m.movie_id = r.movie_id WHERE m.movie_id = $id AND r.user_id = (SELECT user_id FROM users WHERE username = '$user')";
                                        $stmt2 = $pdo->prepare($ratings);
                                        $stmt2->execute();
                                        $mov_ratings = $stmt2->fetchAll();
                                        
                                        // Final Display
                                        $m = $mov_ratings[0][0];
                                        echo "<h3 style='margin-top: 0px;''> Your Rating: $m </h3>";
